...
//       console.log('📡 Received connection test, responding:', data);
//       newSocket.emit('connection-test-response', { 
//         ...data, 
...
// hooks/useSocket.js - Enhanced socket hook with WebRTC support and NAT traversal
import { useEffect, useState, useCallback, useRef } from 'react';
import { io } from 'socket.io-client';
...
export const useSocket = (serverUrl = null) => {
  ...
  const getWebRTCConfig = useCallback(() => {
    return {
      iceServers: [
        ...
        // If you have TURN servers (recommended for production), add them here:
        // {
  ...
  useEffect(() => {
    ...
    newSocket.on('network-quality', (data) => {
      console.log('📊 Network quality update:', data);
    });
  ...
  const emit = useCallback((event, data, callback) => {
    if (socketRef.current && isConnected) {
      ...
      // Add WebRTC config to relevant events
      if (event === 'join-room' && !data.webrtcConfig) {
        data.webrtcConfig = getWebRTCConfig();
      }
  ...
  const runNetworkDiagnostics = useCallback(async () => {
    ...
    try {
      ...
      pc.createOffer()
        ...
        .catch(err => console.warn('STUN test failed:', err));
      
      setTimeout(() => {
        diagnostics.stunConnectivity = pc.iceGatheringState === 'complete';
        pc.close();
      ...
      diagnostics.stunConnectivity = false;
    }
  ...
  }, [isConnected, connectionError, reconnectAttempts, getServerUrl, getWebRTCConfig]);
